...
const APP_URL = "https://dealdash2.netlify.app"; // Your app URL
...
serve(async (req) => {
  ...
  try {
    ...
    const approveUrl = `${APP_URL}/api/invoice-approve?id=${invoice_id}&email=${encodeURIComponent(client_email)}`;
    const rejectUrl = `${APP_URL}/api/invoice-reject?id=${invoice_id}&email=${encodeURIComponent(client_email)}`;
        ...
        attachments: [
          {
            ...
            content: btoa(pdfHtml), // Base64 encode the HTML (in production, use a proper PDF library)
