...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "approve") {
      // Approve the invoice
      const { error: updateError } = await supabase.rpc("approve_invoice", {
        p_invoice_id: invoiceId,
        p_approved_by_email: email,
      });
      ...
      // Reject the invoice
      const { error: updateError } = await supabase.rpc("reject_invoice", {
        p_invoice_id: invoiceId,
        p_rejected_by_email: email,
        p_notes: "Rejected by client via email",
      });
